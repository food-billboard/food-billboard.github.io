...
```typescript
  ...
  function dailyFunction() {
    ...
    // 是否今日开启交易
    const lastTradeNote = DATASOURCE[DATASOURCE.length - 2]
    const currentTradeNote = DATASOURCE[DATASOURCE.length - 1]
    ...
    // 记录今日数据
      ...
      DATASOURCE[DATASOURCE.length - 1].dailyData.push(newDailyData)
    }
