## vscode配置
  ...
  4. 下载[glslang官网](https://github.com/KhronosGroup/glslang/releases)的`glslang-main-osx-Release.zip`  
  5. 解压`4`，把其中的`lib`、`include`、`bin`移动到`/usr/local`下  
  6. 修改`vscode`的配置，`cmd + shift + p`打开配置，扩展中找到`glsl-lint`的`glslang validator path`，将`5`的路径复制到这里（`/usr/local/bin/glslangValidator`）  
  7. 复制[此地址](https://gist.github.com/lewislepton/8b17f56baa7f1790a70284e7520f9623)配置或者直接下面复制配置，粘贴到`vscode`的`User Snippets`的`glsl.json`中。  
  ```json
  /*
    ...
    */
    ...
    @VICTORVICKIE
    VICTORVICKIE commented on Apr 18, 2021
    Sublime Text equivalent :
    save as glsl_completion.sublime-completions in User sublime folder, example : AppData\Roaming\Sublime Text 3\Packages\User

    {
      ...
      [
        ...
        "uniform",
        {
          "contents": "uniform samplerCube ${NAME};",
          "trigger": "samplerCube"
        },
        "gl_FrontFacing",
        {
        ...
        "gl_MaxTextureImageUnits",
        {
          "contents": "const float PI = 3.14159265359;",
          "trigger": "PI"
        },
        {
          "contents": "distance($2, $3)",
          "trigger": "distance"
        },
        "out",
        "texture2D",
        "attribute",
        {
          "contents": "#ifdef GL_ES\nprecision mediump float;\n#endif\n\nuniform float u_time;\nuniform vec2 u_resolution;\nuniform vec2 u_mouse;\n\nvoid main(){\n\tvec2 coord = gl_FragCoord.xy;\n\tvec3 color = vec3(0.0);\n\n\tgl_FragColor = vec4(color, 1.0);\n}",
          "trigger": "simple setup"
        },
        {
          "contents": "cross($2, $3, $4)",
          "trigger": "cross"
        },
        {
          "contents": "sign($2)",
        ...
        {
          "contents": "bool ${NAME} = $3;",
          "trigger": "bool"
        },
        {
          "contents": "float noise1d(float v){\n\treturn cos(v + cos(v * 90.1415) * 100.1415) * 0.5 + 0.5;\n}",
          "trigger": "noise1d"
        },
        {
          "contents": "float ${NAME}(){\n\t\n}",
        ...
        {
          "contents": "uniform sampler2D ${NAME};",
          "trigger": "sampler2D"
        },
        {
          "contents": "length($2)",
        ...
        {
          "contents": "float ${NAME}(vec2 position, float radius){\n\treturn step(radius, length(position - vec2(0.5)));\n}",
          "trigger": "circle shape"
        },
        {
          "contents": "int ${NAME} = $3;",
          "trigger": "int"
        ...
        "gl_PointCoord",
        {
          "contents": "float ${NAME}(vec2 position, float radius, float sides){\n\tposition = position * 2.0 - 1.0;\n\tfloat angle = atan(position.x, position.y);\n\tfloat slice = PI * 2.0 / sides;\n\treturn step(radius, cos(floor(0.5 + angle / slice) * slice - angle) * length(position));\n}",
          "trigger": "polygon shape"
        },
        "gl_FragCoord",
        {
          "contents": "mat2 scale(vec2 scale){\n\treturn mat2(scale.x, 0.0, 0.0, scale.y);\n}",
          "trigger": "scale"
        },
        "gl_MaxFragmentUniformVectors",
        {
          "contents": "inversesqrt($2)",
          "trigger": "inversesqrt"
        },
        {
          "contents": "if($2){\n\t\n} else {\n\t\n}",
        ...
        {
          "contents": "reflect($2, $3)",
          "trigger": "reflect"
        },
        {
          "contents": "mat2 rotate(float angle){\n\treturn mat2(cos(angle), -sin(angle), sin(angle), cos(angle));\n}",
          "trigger": "rotate"
        },
        {
          "contents": "modf($2, $3)",
        ...
        {
          "contents": "roundEven($2)",
          "trigger": "roundEven"
        },
        "varying",
        {
          "contents": "smoothstep($2, $3, $4)",
          "trigger": "smoothstep"
        },
        {
          "contents": "discard;",
        ...
        {
          "contents": "float ${NAME}(vec2 position, vec2 scale){\n\tscale = vec2(0.5) - scale * 0.5;\n\tvec2 shaper = vec2(step(scale.x, position.x), step(scale.y, position.y));\n\tshaper *= vec2(step(scale.x, 1.0 - position.x), step(scale.y, 1.0 - position.y));\n\treturn shaper.x * shaper.y;\n}",
          "trigger": "rectangle shape"
        },
        {
          "contents": "for(int $2 = 0; $2 < $3; $2++){\n\t\n}",
          "trigger": "for"
        },
        {
          "contents": "float random2d(vec2 coord){\n\treturn fract(sin(dot(coord.xy, vec2(12.9898, 78.233))) * 43758.5453);\n}",
          "trigger": "random2d"
        },
        {
          "contents": "asinh($2)",
...
```glsl
  ...
  vec2 vec2_pos = vec2(1.0, 2.0);
  vec2_pos.x = 3.0;
  vec3 vec3_pos = vec3(1.0, 2.0, 3.0);
